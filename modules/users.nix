{ ... }:
{
  users.extraUsers.bag = {
    description = "Mathias Rüdiger";
    home = "/home/bag";
    isNormalUser = true;
    uid = 1000;
    extraGroups = [ "wheel" "networkmanager" "input" "video" ];
    openssh.authorizedKeys.keys = [
      "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIKIz573Iv8nzMWdKAGBxVN9yydoEHHp62v0JwVu5IawD root@josephine"
      "ssh-rsa  bag@butterfly"
      "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIK9FWHQm8CyMXejjV4Tn99qA0qe/OFTDE3MKbnCxeQ9V root@farting-unicorn"
    ];
  };

  security = {
    sudo = {
      enable = true;
    };
  };

  home-manager.users.bag = { };
}
